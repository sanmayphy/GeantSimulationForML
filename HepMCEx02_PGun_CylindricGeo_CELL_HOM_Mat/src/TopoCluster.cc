...
TopoCluster::TopoCluster(const float (&TotalEnergyArray)[6][kMaxPixel][kMaxPixel], const float (&NoiseArray)[6][kMaxPixel][kMaxPixel], const float (&ChargeEnergyArray)[6][kMaxPixel][kMaxPixel], const float (&NeutralEnergyArray)[6][kMaxPixel][kMaxPixel], const int (&TopoLayersPix)[6])
{
    ...
    for (int l = 0; l < 6; l++)
    {
        ...
        for (int e = 0; e < kMaxPixel; e++)
        {
            ...
            {
                CellArray[l][e][p].update(0, 0, 0, 0, l, e, p, 0, false, TopoLayersPix);
                float CellTotSignal = TotalEnergyArray[l][e][p] + NoiseArray[l][e][p];
                CellArray[l][e][p].SetSigma(CellTotSignal/NoiseValues[l]);
                CellArray[l][e][p].SetTotEnergy(CellTotSignal);
                CellArray[l][e][p].SetNuEnergy(NeutralEnergyArray[l][e][p]);
                CellArray[l][e][p].SetChEnergy(ChargeEnergyArray[l][e][p]);
            }
        }
    }
}
...
void TopoCluster::FillNoise(float (&Noise)[6][kMaxPixel][kMaxPixel])
{
    for (int i = 0; i < 6; i++)
    {
        for (int j = 0; j < kMaxPixel; j++)
        {
            for (int k = 0; k < kMaxPixel; k++)
            {
                if (CellArray[i][j][k].GetLabel()!=0) Noise[i][j][k] = Noise[i][j][k];
                else Noise[i][j][k] = 0;
            }
        }
    }
...
void TopoCluster::Run()
{
    ...
    int count  = 0;
    float dEta[6]; 
    // = {d_eta*(kMaxPixel/LPixel[0]),   d_eta*(kMaxPixel/LPixel[1]),   d_eta*(kMaxPixel/LPixel[2]),
    //                 d_eta*(kMaxPixel/LPixel[3]), d_eta*(kMaxPixel/LPixel[4]), d_eta*(kMaxPixel/LPixel[5])};
    float dPhi[6];
    //  = {divided_tube_dPhi*(kMaxPixel/LPixel[0]),   divided_tube_dPhi*(kMaxPixel/LPixel[1]),   divided_tube_dPhi*(kMaxPixel/LPixel[2]),
    //                 divided_tube_dPhi*(kMaxPixel/LPixel[3]), divided_tube_dPhi*(kMaxPixel/LPixel[4]), divided_tube_dPhi*(kMaxPixel/LPixel[5])};
    ...
    for (int i = 0; i < 6; i++)
    {
        dEta[i] = d_eta*(kMaxPixel/LPixel[i]);
        dPhi[i] = divided_tube_dPhi*(kMaxPixel/LPixel[i]);
