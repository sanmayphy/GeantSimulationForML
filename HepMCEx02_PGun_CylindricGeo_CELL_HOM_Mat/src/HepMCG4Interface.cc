                                ...
                                G4Event* g4event)
{
  ...
  runAction->RealParticalInfo.clear();
    ...
    for (HepMC::GenVertex::particle_iterator
         ...
         vpitr != (*vitr)->particles_end(HepMC::children); ++vpitr) 
        {
          ...
          G4PrimaryParticle* g4prim=
            new G4PrimaryParticle(pdgcode, p.x()*MeV, p.y()*MeV, p.z()*MeV);//, p.t()*GeV
          vector <double> vec;
          vec.push_back(pdgcode);
          vec.push_back(p.x()*MeV);
          vec.push_back(p.y()*MeV);
          vec.push_back(p.z()*MeV);
          vec.push_back(p.t()*MeV);
          //* Saved information about Real particle that was produced in the process.
          runAction->RealParticalInfo.push_back(vec);
          G4cout << " pos.px " << pos.px()<< " pos.py " <<pos.py() << " pos.pz "<< pos.pz()<<" pos.e() "<<pos.e()<<G4endl;
          G4cout << " pdgcode " << pdgcode << "  p.x()*GeV " << p.x()*GeV << " p.y()*GeV "<< p.y()*GeV <<" p.z()*GeV "<<p.z()*GeV<<" p.t()*GeV "<<p.t()*GeV<<G4endl;
          G4cout << "pdgcode " << pdgcode << " pos.e() "<< pos.e() <<" EnG4 "<<g4prim->GetTotalEnergy()<<G4endl;
          numbstab++;
          g4prim->Print();
          g4vtx-> SetPrimary(g4prim);
          G4cout << "GetNumberOfParticle " << g4vtx->GetNumberOfParticle()<<G4endl;
        }
    G4cout<<"RealParticalInfo size "<<runAction->RealParticalInfo.size()<<G4endl;
